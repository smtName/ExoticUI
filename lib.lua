...
local ExoticLibrary = {
	...
	Theme = {
		Default = {
			...
			SliderBackground = Color3.fromRGB(34, 34, 34),
			SliderProgress = Color3.fromRGB(43, 105, 159),
			SliderStroke = Color3.fromRGB(48, 119, 177),
			
			ToggleBackground = Color3.fromRGB(34, 34, 34),
...
local ExoticUI = game:GetObjects("rbxassetid://10804731440")[1]



if gethui then
	ExoticUI.Parent = gethui()
elseif syn.protect_gui then 
	syn.protect_gui(ExoticUI)
	ExoticUI.Parent = CoreGui
elseif CoreGui:FindFirstChild("RobloxGui") then
	ExoticUI.Parent = CoreGui:FindFirstChild("RobloxGui")
else
	ExoticUI.Parent = CoreGui
end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == ExoticUI.Name and Interface ~= ExoticUI then
			Interface.Enabled = false
			Interface.Name = "ExoticUI-Old"
		end
	end
else
...
local Main = ExoticUI.Main
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList

ExoticUI.DisplayOrder = 100
...
local Notifications = ExoticUI.Notifications

local SelectedTheme = ExoticLibrary.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = ExoticLibrary.Theme[ThemeName]
	for _, obj in ipairs(ExoticUI:GetDescendants()) do
	...
	ExoticUI.Main.BackgroundColor3 = SelectedTheme.Background
	ExoticUI.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	ExoticUI.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	ExoticUI.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow
	
	ExoticUI.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	ExoticUI.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	ExoticUI.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	table.foreach(Data, function(FlagName, FlagValue)
		if ExoticLibrary.Flags[FlagName] then
			spawn(function() 
				if ExoticLibrary.Flags[FlagName].Type == "Colorpicker" then
					ExoticLibrary.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if ExoticLibrary.Flags[FlagName].CurrentValue or ExoticLibrary.Flags[FlagName].CurrentKeybind or ExoticLibrary.Flags[FlagName].CurrentOption ~= FlagValue then ExoticLibrary.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
			ExoticLibrary:Notify({Title = "Flag Error", Content = "ExoticUI was unable to find '"..FlagName.. "'' in the current script"})
		end
	end)
end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(ExoticLibrary.Flags) do
...
function ExoticLibrary:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= ExoticLibrary.Theme.Default then
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("ExoticUI | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
		...
		if ExoticUI.Name == "ExoticUI" then
...
function Hide()
	Debounce = true
	ExoticLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping RightShift", Duration = 7})
...
function ExoticLibrary:CreateWindow(Settings)
	...
	pcall(function()
		...
		if not isfolder(ExoticFolder.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(ExoticFolder.."/Discord Invites") then
			makefolder(ExoticFolder.."/Discord Invites")
		end
		if not isfile(ExoticFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(ExoticFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"ExoticUI RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(ExoticFolder.."/Key System") then
			makefolder(ExoticFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			...
			if not Success then
				print("ExoticUI | "..Settings.KeySettings.Key.." Error " ..tostring(Response))
			end
		end
		...
		if isfile(ExoticFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			if readfile(ExoticFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) == Settings.KeySettings.Key then
				Passthrough = true
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				if KeyMain.Input.InputBox.Text == Settings.KeySettings.Key then
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(ExoticFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, Settings.KeySettings.Key)
						end
						ExoticLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
					end
				else
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("ExoticUI | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("ExoticUI | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					if Dropdown.Selected.Text ~= Option then
						...
						if not Success then
							...
							print("ExoticUI | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					ExoticLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("ExoticUI | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					ExoticLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("ExoticUI | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					ExoticLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if Dragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("ExoticUI | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					ExoticLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
...
function ExoticLibrary:Destroy()
	ExoticUI:Destroy()
end
...
function ExoticLibrary:LoadConfiguration()
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				LoadConfiguration(readfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension))
				ExoticLibrary:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end
